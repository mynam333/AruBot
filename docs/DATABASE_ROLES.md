...
`public.bot_counter_values`. Migration `022_bot_counter_values.sql` creates the table but
does not broaden database access for `public`; deployments with a separate runtime role
must grant those four table privileges through their normal role-provisioning policy.
