...
type AdminStrokePoint = { x: number; y: number; p?: number; t?: number; replayT?: number };
...
function drawAdminCrayonGrain(ctx: CanvasRenderingContext2D, x: number, y: number, nx: number, ny: number, lineWidth: number, color: string, alpha: number, seed: number) {
  const half = lineWidth * 0.55;
  const grains = Math.max(5, Math.min(26, Math.ceil(lineWidth * 0.42)));
  for (let i = 0; i < grains; i += 1) {
    const a = adminCrayonNoise(seed + i * 7.13);
    const b = adminCrayonNoise(seed + i * 11.71);
    const c = adminCrayonNoise(seed + i * 17.31);
    ...
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.ellipse(x + nx * offset - ny * along, y + ny * offset + nx * along, radius * (1.1 + b), radius * (0.55 + a * 0.75), Math.atan2(ny, nx), 0, Math.PI * 2);
    ctx.fill();
  }
}

function drawAdminCrayonPath(ctx: CanvasRenderingContext2D, points: AdminStrokePoint[], width: number, height: number, lineWidth: number, color: string, alpha: number) {
  ctx.save();
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ...
  if (points.length === 1) {
    drawAdminCrayonGrain(ctx, points[0].x * width, points[0].y * height, 0, 1, lineWidth, color, alpha, points[0].x * 997 + points[0].y * 577);
    ctx.restore();
    return;
  }
  for (let index = 1; index < points.length; index += 1) {
    ...
    for (let i = 0; i <= count; i += 1) {
      ...
      drawAdminCrayonGrain(ctx, x0 + dx * t, y0 + dy * t, nx, ny, lineWidth * pressure, color, alpha, seed);
    }
  }
  ctx.restore();
}

function drawAdminBrushDab(ctx: CanvasRenderingContext2D, x: number, y: number, tx: number, ty: number, lineWidth: number, color: string, alpha: number, seed: number) {
  const nx = -ty;
  const ny = tx;
  const spread = lineWidth * (0.78 + adminCrayonNoise(seed + 0.21) * 0.38);
  const length = lineWidth * (0.58 + adminCrayonNoise(seed + 0.47) * 0.46);
  const angle = Math.atan2(ty, tx);
  ctx.globalCompositeOperation = 'source-over';
  ctx.fillStyle = color;
  ctx.globalAlpha = alpha * (0.1 + adminCrayonNoise(seed + 0.73) * 0.06);
  ctx.beginPath();
  ctx.ellipse(x, y, length * 1.05, spread * 1.28, angle, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = alpha * (0.42 + adminCrayonNoise(seed + 1.17) * 0.18);
  ctx.beginPath();
  ...
  for (let i = 0; i < bristles; i += 1) {
    const a = adminCrayonNoise(seed + i * 5.91);
    const b = adminCrayonNoise(seed + i * 9.37);
    ...
    ctx.strokeStyle = color;
    ctx.lineWidth = Math.max(0.7, lineWidth * (0.018 + adminCrayonNoise(seed + i * 13.11) * 0.035));
    ctx.beginPath();
...
function drawAdminInkBrushPath(ctx: CanvasRenderingContext2D, points: AdminStrokePoint[], width: number, height: number, lineWidth: number, color: string, alpha: number) {
  ctx.save();
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ...
  if (points.length === 1) {
    drawAdminBrushDab(ctx, points[0].x * width, points[0].y * height, 1, 0, lineWidth * 0.85, color, alpha, points[0].x * 701 + points[0].y * 1301);
    ctx.restore();
    return;
  }
  for (let index = 1; index < points.length; index += 1) {
    ...
    for (let i = 0; i <= count; i += 1) {
      const t = i / count;
      const wobble = (adminCrayonNoise(index * 97 + i * 19.3) - 0.5) * segmentWidth * 0.16;
      const nx = -ty;
      const ny = tx;
      const seed = index * 211 + i * 17 + Math.floor((Number(prev.t || 0) + Number(point.t || 0)) * 0.01);
      drawAdminBrushDab(ctx, x0 + dx * t + nx * wobble, y0 + dy * t + ny * wobble, tx, ty, segmentWidth * (0.9 + adminCrayonNoise(seed) * 0.26), color, alpha, seed);
    }
  }
  ctx.restore();
}
...
function drawAdminStroke(ctx: CanvasRenderingContext2D, stroke: AdminStroke, width: number, height: number, atMs = Infinity) {
  ...
  if (brush.type === 'crayon') {
    drawAdminCrayonPath(ctx, points, width, height, lineWidth, String(brush.color || '#ff6b9a'), Math.max(0.05, Math.min(1, Number(brush.alpha ?? 1) || 1)));
    ctx.restore();
    return;
  }
  if (points.length === 1) {
  ...
  if (brush.type === 'brush' || brush.type === 'highlighter') {
    ...
    drawAdminInkBrushPath(ctx, points, width, height, lineWidth, String(brush.color || '#ff6b9a'), alpha);
    ctx.restore();
    return;
  }
  ctx.beginPath();
