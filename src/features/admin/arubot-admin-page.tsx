...
export function ArubotAdminPage() {
  ...
  useEffect(() => {
    ...
    const recognized = (auth === 'error' && reason === 'admin_required')
      ...
      || reason === 'central_bot_register_channel'
    ...
    if (auth === 'error' && reason === 'quota_exceeded') toast.error('YouTube 봇 채널 자동 확인을 완료하지 못했습니다. 다시 연결한 뒤 채널을 직접 등록해 주세요.');
    if (reason === 'central_bot_select_channel') toast.info('YouTube 봇으로 사용할 채널을 선택해 주세요.');
    if (reason === 'central_bot_register_channel') toast.info('Google 인증이 완료되었습니다. YouTube 봇 채널 URL을 직접 등록해 주세요.');
